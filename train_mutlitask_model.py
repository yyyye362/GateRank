...
class MultiTaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt_enc = self.tokenizer(prompt, truncation=True, max_length=self.max_src_len, padding=False, return_tensors='pt')
        prompt_input_ids = prompt_enc.input_ids.squeeze(0)
        full_enc = self.tokenizer(full_text, truncation=True, max_length=self.max_seq_len, padding='max_length', return_tensors='pt')
...
class MultiTaskModel(nn.Module):
    def __init__(self, model_name='Qwen2.5-Coder-1.5B-Instruct', num_labels=2, lora_r=8, lora_alpha=32, lora_dropout=0.1, use_gradient_checkpointing=True):
        super().__init__()
    ...
    def forward(self, prompt_input_ids, input_ids, attention_mask, labels, cls_label):
        ...
        cls_output = self.model(input_ids=prompt_input_ids, output_hidden_states=True, return_dict=True)
        last_hidden = cls_output.hidden_states[-1]
        last_hidden = last_hidden.to("cuda:0")
        pooled_output = last_hidden.mean(dim=1)
        cls_logits = self.classifier(pooled_output)
        gen_output = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, return_dict=True)
        gen_loss = gen_output.loss
        cls_loss = F.cross_entropy(cls_logits, cls_label.to("cuda:0"))
...
def train(train_path, model_name, batch_size, epochs, lr, cls_weight, gen_weight, output_dir, lora_r, lora_alpha, lora_dropout, grad_accum_steps, max_grad_norm, warmup_ratio, log_interval, max_source_length, max_target_length):
    ...
    model = MultiTaskModel(model_name=model_name, lora_r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, use_gradient_checkpointing=False)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in progress_bar:
            ...
            cls_logits, gen_loss, cls_loss = model(prompt_input_ids=prompt_input_ids, input_ids=input_ids, attention_mask=attention_mask, labels=labels, cls_label=cls_labels)
            ...
            if (step + 1) % log_interval == 0:
                progress_bar.set_postfix({'loss': loss.item(), 'cls_loss': cls_loss.item(), 'gen_loss': gen_loss.item()})
        ...
        logger.info(f"Epoch {epoch}/{epochs} | Train Loss: {avg_loss:.4f} | CLS Loss: {avg_cls_loss:.4f} | GEN Loss: {avg_gen_loss:.4f} | CLS Acc: {cls_acc:.4f}")
